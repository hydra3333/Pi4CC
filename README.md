...
---
   ...
   with manually-configured IIS (it is easy) and a couple of Pi4CC files manually edited.  No automated install script though. 
...
1. Download/Extract/Edit source files
...
Ensure you have a fixed IPv4 address ... usually use you roter's DHCP settings and your PC's mac address to reserve one.
...
Find the line containing "http://10.0.0.6/Pi4CC/imagefiles/free-boat_02.jpg" and change the IP address to YOUR fixed IP addresss, eg "http://10.0.0.6/Pi4CC/imagefiles/free-boat_02.jpg"
...
3. Configure IIS in Windows 10

Click Start, type the word "IIS" without quotes and see "Internet Information Services (IIS) Manager" appear,
...
(in a CMD window, use command IPCONFIG to check the IPv4 address xx.xx.xx.xx).
...
a) Type: "https", IP Address: "All Unassigned",               Port: "443", Host name: <host name, eg 3900X>
b) Type: "https", IP Address: <the IPv4 address xx.xx.xx.xx>, Port: "443", Host name: <leave it empty>
c) Type: "http",  IP Address: "All Unassigned",               Port: "80",  Host name: <host name, eg 3900X>
d) Type: "http",  IP Address: <the IPv4 address xx.xx.xx.xx>, Port: "80",  Host name: <leave it empty>
...
Note 2: if you used HTTPS, the chromecast icon will be present (if you use plain http it will not appear ... google required it).
