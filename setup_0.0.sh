...
echo ""
echo "# Add 30 seconds for the USB3 drive to spin up.  Add the line at the top."
...
sudo sed -i "$ a #UUID=${server_USB3_DEVICE_UUID} ${server_root_USBmountpoint} ntfs defaults,auto,users,rw,exec,umask=000,dmask=000,fmask=000,uid=1000,gid=1000,noatime,nodiratime,x-systemd.device-timeout=120 0 0" "/etc/fstab"
sudo sed -i "$ a UUID=${server_USB3_DEVICE_UUID} ${server_root_USBmountpoint} ntfs defaults,auto,users,rw,exec,umask=000,dmask=000,fmask=000,uid=1000,gid=1000,noatime,nodiratime,x-systemd.device-timeout=120 0 0" "/etc/fstab"
set +x
