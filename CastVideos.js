...
CastPlayer.prototype.addVideoList = function () {
  ...
  for (var i = 0; i < this.mediaContents.length; i++) {                 // iterate every JSON entry
	...
	tmpstring = '<div class="lineitem">' + '<div class="lineitem_lefttext">' + this.mediaContents[i]['title']  + " (" + this.mediaContents[i]['resolution'] + "," + this.mediaContents[i]['video_codec']  + "/" +  this.mediaContents[i]['audio_codec'] + ")[" + this.mediaContents[i]['duration'] + "]" + '</div><div class="lineitem_righttext">' + encodeURI(this.mediaContents[i]['sources'][0]) + '</div></div>' ;
